...
class BannerIndicater extends Component {
    ...
    render() {
        ...
        case IndicaterType.number: {
            elements.push((
                <Text key={0} style={[styles.number, numberStyle]}>{pageIndex + 1 + ' / ' + count}</Text>));
            break;
        }
        ...
        case IndicaterType.dot: {
            for (let i = 0; i < count; i++) {
                if (i === pageIndex) {
                    elements.push((
                        <View key={i} style={[styles.dot, {backgroundColor: '#fff'}, currentDotStyle]} />));
                } else {
...
export default class TBanner extends Component {
    ...
    componentDidMount() {
        ...
        if (autoPlay && this.elementCount > 1) {
            this._makeTimer();
        }
    }
    ...
    _onMomentumScrollEnd = (e) => {
        ...
        this.indicater && this.indicater.updatePageIndex(this.pageindex);
    };
    ...
    _goIndex = (index, animated = false) => {
        ...
        this.props.onItemChange && this.props.onItemChange(index - 1);
    };
        ...
        return (
            <View style={[styles.banner, style]}>
                <ScrollView
                    ...
                    pagingEnabled={true}
                    bounces={false}
                    scrollEnabled={this.elementCount > 1}
